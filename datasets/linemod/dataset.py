...
class PoseDataset(data.Dataset):
    def __init__(self, mode, num, add_noise, root, noise_trans, refine,is_spherical):
    ...
    def __getitem__(self, index):
        ...
        b = self.xmap[rmin:rmax, cmin:cmax]
        ...
        pt3 = []
        for i in range(self.num_pt_mesh_large):
            pt3.append(i)
        pt3 = np.array(pt3).reshape(self.num_pt_mesh_large,1)

        if self.add_noise:
            cloud = np.add(cloud, add_t)
        if self.is_spherical:
            cloud,R = spherical.get_Spherical_coordinate(cloud,normalized=True)
